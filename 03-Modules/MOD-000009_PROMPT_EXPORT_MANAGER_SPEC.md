# Prompt Export Manager Module Specification

Document ID: MOD-000009  
Version: 0.1.0  
Status: Draft  
Owner: Aurora Labs  
Repository: aurora-spec  
Path: 03-Modules/MOD-000009_PROMPT_EXPORT_MANAGER_SPEC.md  
...
The Prompt Export Manager module exists to convert approved AFL structures and Prompt Export Intent into external export artifacts through provider-independent rules and provider-specific plugins.
...
- Prepare prompt export requests from AFL structures
- Validate Prompt Export Intent before export
- Coordinate with provider plugins through approved interfaces
- Keep export artifacts separate from AFL source meaning
- Support generic human-readable export formats
- Respect safety constraints and reference asset rules
- Emit prompt export lifecycle events
...
- Owning Scene meaning
- Owning Shot meaning
- Modifying AFL core structures
- Executing online provider calls directly without plugin boundary
- Creating provider SDKs
- GUI prompt editor rendering
...
- Provider target selection metadata where approved
...
The Prompt Export Manager module defines a controlled responsibility boundary inside Aurora.
